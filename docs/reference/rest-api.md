...
- [ClickHouse RowBinary](https://clickhouse.tech/docs/en/interfaces/formats/#rowbinary). If `partition_by_field` is set, Querent returns chunks of data for each partition field value. Each chunk starts with 16 bytes being partition value and content length and then the `fast_field` values in `RowBinary` format.
...
Depending on the client, the trailer header with error details may not be shown. The error will also be logged in querent ("Error when streaming search results").
...
| Variable              | Type               | Description                                                                                                           | Default value                         |
|-----------------------|--------------------|-----------------------------------------------------------------------------------------------------------------------|---------------------------------------|
| `version`          | `String`           | Config format version, use the same as your Querent version. (mandatory)                                             |                                       |
...
| Variable          | Type     | Description                                                                                          | Default value |
|-------------------|----------|------------------------------------------------------------------------------------------------------|---------------|
| `version**       | `String` | Config format version, put your current Querent version. (mandatory)                                |               |
...
```json title="source_config.json
{
    ...
    "source_type": "kafka",
    "params": {
        "topic": "querent-fts-staging",
        "client_params": {
            "bootstrap.servers": "kafka-querent-server:9092"
        }
    }
}
```
